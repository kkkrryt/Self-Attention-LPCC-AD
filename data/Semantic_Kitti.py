...
class SemanticKITTIDataset(Dataset):
    def __init__(self, data_root="/home/fengmj8/datasets/SemanticKITTIDataset/", split="train", ignore_index=-1, sample_ratio=-1):
...
if __name__ == "__main__":
    ...
    print(len(dataset))
    pcs = []
    tq = tqdm(list(range(len(dataset))))
    for i in tq:
        pc = dataset[i]
        pcs.append(len(pc))
        tq.set_description(f"{max(pcs)}  {min(pcs)}")
    pcs = np.array(pcs)
    print(pcs.max(), pcs.min())